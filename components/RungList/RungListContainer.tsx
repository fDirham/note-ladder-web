...
import {
  cursorMapStateType,
  useCursorMapState,
} from "globalStates/useCursorMapStore"
...
export default function RungListContainer(props: RungListContainerProps) {
  ...
  const cursorMapState: cursorMapStateType = useCursorMapState()
  ...
  useEffect(() => {
    const { id } = props.parentRung
    const storedCursor = cursorMapState.cursorMap[id]
    if (storedCursor) setCursor(storedCursor)
    else setCursor(0)
  }, [props.parentRung.id])

  useEffect(() => {
    if (!props.parentRung) return
    const { id } = props.parentRung
    cursorMapState.setRung(id, cursor)
  }, [cursor])
